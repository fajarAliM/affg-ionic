...
import { GolfsMapPage } from '../golfsMap/golfsMap';
...
@Component({
  selector: 'page-golfs',
  templateUrl: 'golfs.html',
})
export class GolfsPage {
...
openMap(golfs){
	console.log("Esto si");
	  	this.navCtrl.setRoot(GolfsMapPage,{golfs: golfs});
...
openMenu(id) {
  ...
  getGolfs(){
    ...
    //this.http.get(this.global.url+"/clubs/2019", {headers}).subscribe((resp:any) => {
	    ...
	    this.http.get(this.global.urlApiLocal+"/getData.php?e=golfs&s=&l="+this.perPage+"&p="+this.page+"&token="+encodeURIComponent(this.global.token)).subscribe((resp:any) => {
