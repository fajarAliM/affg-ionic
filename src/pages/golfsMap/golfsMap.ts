...
@Component({
  selector: 'page-golfsMap',
  templateUrl: 'golfsMap.html',
})
export class GolfsMapPage {
...
map: any;
  ...
  ionViewDidEnter(){
	  ...
	  this.getGolfs().then(data => { //Necesito el token, sin él no hago nada
...
popupAnchor: [0, -40]
  ...
  loadmap() {
    ...
    if(centroactual.lat==0 && centroactual.lng==0){ //No hay centro definido, cojo Paris
  ...
  getGolfs(){
  	...
  	return new Promise(resolve => {
			  ...
			  'X-Auth-Token': this.global.token
			    ...
			    this.http.get(this.global.urlApiLocal+"/getData.php?e=golfs&s=&l=300&p=1&token="+encodeURIComponent(this.global.token)).subscribe((resp:any) => {
